...
if(isset($_POST['validation']) && !empty($_POST['validation'])) {
	...
	if($responseData->success) {
		...
		// compose email body
		$body = '<h1>' . $name .' Has Registered An Interest In '. $service .' </h1>
                <h2>Here are their details:</h2>
                <p>Name: '. $name .' </p> 
                <p>Email: '. $email .' </p>
                <p>Phone No.: '. $phone .' </p>
                <p>Service: '. $service .' </p>
        ...
        <div id='success_page'>
            ...
            <p>Thank you <strong>$name</strong>for registering an interest in  $service  , I will keep you up to date when this becomes available.</p>
        </div>
    </fieldset>";
